...
from ..constants.stacks import VALID_LEVELS, VALID_STACKS, VALID_TOPICS
...
def create_session():
    ...
    # Validamos que el stack, el nivel y el tema pertenecen a las listas permitidas
    if (
        stack not in VALID_STACKS
        or level not in VALID_LEVELS
        or topic not in VALID_TOPICS.get(stack, set())
    ):
        return jsonify({"Error": "El stack, el nivel o el tema seleccionados no estan permitidos"}), 400
